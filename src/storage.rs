...
pub fn detect_layout(path: &Path) -> Layout {
    if path.is_dir() { Layout::Directory } else { Layout::Single }
}
...
pub fn save_directory(root: &Path, project: &Project) -> Result<()> {
    ...
    root_obj.insert("_instructions".into(), Value::Array(
        instructions_block().into_iter().map(Value::String).collect()));
    ...
    root_obj.insert("requirement_ids".into(), Value::Array(
        project.requirements.keys().map(|k| Value::String(k.clone())).collect()));
...
pub fn load_directory(root: &Path, force: bool) -> Result<Project> {
    ...
    let stored_hash = root_obj.remove("_integrity")
        .and_then(|v| v.as_str().map(|s| s.to_string()))
    ...
    if let Ok(entries) = fs::read_dir(&reqs_dir) {
        for e in entries.flatten() {
            let p = e.path();
            if p.extension().and_then(|s| s.to_str()) != Some("req") { continue; }
            let body = fs::read_to_string(&p)?;
            let req: crate::model::Requirement = serde_json::from_str(&body)
    ...
    let project = Project {
        name: root_obj.get("name").and_then(|v| v.as_str()).unwrap_or("").to_string(),
        ...
        next_id: root_obj.get("next_id").and_then(|v| v.as_u64()).unwrap_or(1) as u32,
        requirements,
    };
...
fn directory_integrity(project: &Project) -> Result<String> {
    ...
    for (_id, req) in &project.requirements {
