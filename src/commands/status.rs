...
pub fn run(args: StatusArgs, file: &Option<PathBuf>) -> Result<()> {
    ...
    let delivery_pct = if non_obsolete == 0 { 0.0 } else { 100.0 * done as f64 / non_obsolete as f64 };
    ...
    println!("Total:   {}", total);
    println!();
    println!("  draft       : {:>4}  ({:>5.1}%)", counts[0], pct(counts[0]));
    println!("  proposed    : {:>4}  ({:>5.1}%)", counts[1], pct(counts[1]));
    println!("  approved    : {:>4}  ({:>5.1}%)", counts[2], pct(counts[2]));
    println!("  implemented : {:>4}  ({:>5.1}%)", counts[3], pct(counts[3]));
    println!("  verified    : {:>4}  ({:>5.1}%)", counts[4], pct(counts[4]));
    println!("  obsolete    : {:>4}  ({:>5.1}%)", counts[5], pct(counts[5]));
    println!();
    println!("Delivery progress: {:.1}%  ({} of {} non-obsolete are implemented or verified)",
