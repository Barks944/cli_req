...
fn req_0066_batch_unknown_mutation_kind_is_rejected_atomically() {
    ...
    let out = s.run(&["batch", path.to_str().unwrap()]);
    assert!(!out.status.success());
    assert!(stderr(&out).contains("unknown variant") || stderr(&out).contains("parse batch document"));
}
...
fn req_0067_import_json_with_invalid_shape_reports_clearly() {
    ...
    assert!(!out.status.success());
    assert!(stderr(&out).to_lowercase().contains("array") || stderr(&out).to_lowercase().contains("object"));
}
...
fn req_0067_import_strict_aborts_on_first_invalid_item() {
    ...
    let out = s.run(&["import", "-f", "markdown", path.to_str().unwrap(), "--strict"]);
    assert!(!out.status.success(), "strict mode should abort on the bad item");
}
...
fn req_0017_mcp_tools_list_lists_ten_tools() {
    ...
    for required in &["req_list","req_show","req_add","req_update","req_validate","req_help"] {
        assert!(names.contains(required), "missing tool {}", required);
    }
    assert!(!names.contains(&"req_repair"), "MCP must not expose req_repair");
}
...
fn req_0017_mcp_unknown_method_returns_error_envelope() {
    ...
    assert!(v["error"].is_object(), "unknown method should return error: {}", v);
}
...
fn req_0065_strict_allow_lets_known_orphans_pass() {
    let s = Sandbox::new(); s.init("p");
    let _ = s.run(&[
        "add", "--title", "Verification-only requirement here",
        "--statement", "The system shall be verifiable through inspection only.",
        "--rationale", "No code site needed.",
        "--kind", "constraint", "--priority", "could",
    ]);
    // Without --allow: strict fails
    let blocked = s.run(&["coverage", "--path", s.dir.path().to_str().unwrap(), "--strict"]);
    assert!(!blocked.status.success(), "orphan should trip strict");
    // With --allow REQ-0001: strict passes
    let allowed = s.run(&["coverage", "--path", s.dir.path().to_str().unwrap(),
        "--strict", "--allow", "REQ-0001"]);
    assert!(allowed.status.success(), "explicit allow should clear strict: {}", stderr(&allowed));
}
