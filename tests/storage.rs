...
fn req_0002_init_writes_diffable_json() {
    let s = Sandbox::new(); s.init("test-proj");
    let text = fs::read_to_string(s.path()).unwrap();
    assert!(text.starts_with('{'), "file should start with {{: {}", &text[..40]);
    assert!(text.contains("_format"), "should have _format field");
    assert!(text.contains("\"req-v1\""), "should declare format tag");
...
fn req_0003_integrity_blocks_load_after_semantic_tamper() {
    ...
    s.run(&[
        ...
        "--statement", "The system shall behave reasonably under all conditions tested.",
        "--rationale", "Baseline.",
        "--kind", "constraint", "--priority", "must",
    ]);
    ...
    assert!(stderr(&out).contains("integrity"), "stderr: {}", stderr(&out));
}
...
fn req_0003_integrity_ignores_whitespace_only_change() {
    ...
    let out = s.run(&["list"]);
    assert!(out.status.success(), "whitespace edit should not break integrity: {}", stderr(&out));
}
...
fn req_0005_repair_recovers_after_tamper() {
    ...
    s.run(&[
        "add",
        "--title", "Some workable title",
        "--statement", "The system shall persist data across restarts of the host process.",
...
fn req_0019_save_is_atomic_no_partial_tmp_left_behind() {
    ...
    s.run(&[
        "add",
        "--title", "Atomic write evidence",
        "--statement", "The save path shall produce no partial files visible to readers.",
        "--rationale", "Crash safety.",
        "--kind", "constraint", "--priority", "must",
    ]);
    let entries: Vec<_> = fs::read_dir(s.dir.path()).unwrap()
        .filter_map(|e| e.ok().map(|e| e.file_name().to_string_lossy().into_owned()))
        .collect();
    assert!(entries.iter().any(|n| n == "project.req"), "project.req present");
    assert!(!entries.iter().any(|n| n.ends_with(".tmp")), "no .tmp lingering: {:?}", entries);
}
...
fn req_0010_sequential_ids_no_reuse_after_delete() {
    ...
    for i in 1..=3 {
        ...
        let out = s.run(&[
            "add",
            "--title", &title,
            "--statement", "The system shall do an interesting thing for this test.",
            "--rationale", "Sequence.",
    ...
    let out = s.run(&[
        "add",
        "--title", "Followup after hard delete",
        "--statement", "The system shall continue assigning fresh identifiers without reuse.",
        "--rationale", "No reuse.",
        "--kind", "constraint", "--priority", "could",
    ]);
    assert!(out.status.success(), "stderr: {}", stderr(&out));
    let list = stdout(&s.run(&["list", "--json"]));
    assert!(list.contains("REQ-0004"), "next ID should be REQ-0004, got: {}", list);
}
