...
fn req_0075_init_directory_layout_writes_index_and_requirements_dir() {
    ...
    let out = Command::new(env!("CARGO_BIN_EXE_req"))
        .args(["init", "-n", "dir-proj", "-o", dir.to_str().unwrap(), "--layout", "directory"])
        .output().expect("init");
...
fn req_0075_add_persists_one_file_per_requirement() {
    ...
    Command::new(env!("CARGO_BIN_EXE_req"))
        ...
        .output().expect("init");
    let out = Command::new(env!("CARGO_BIN_EXE_req"))
        .args(["--file", dir.to_str().unwrap(), "add",
              "--title", "Persisted under the directory layout here",
              "--statement", "The system shall write this requirement to its own file under requirements/.",
              "--rationale", "Test fixture.",
              "--kind", "constraint", "--priority", "could"])
        .output().expect("add");
    assert!(out.status.success(), "stderr: {}", String::from_utf8_lossy(&out.stderr));
    assert!(dir.join("requirements/REQ-0001.req").exists(),
...
fn req_0075_integrity_detects_per_file_tamper() {
    ...
    Command::new(env!("CARGO_BIN_EXE_req"))
        .args(["--file", dir.to_str().unwrap(), "add",
              "--title", "Will be tampered with in this test",
              "--statement", "The system shall persist this so we can mutate the file.",
              "--rationale", "Test.",
              "--kind", "constraint", "--priority", "could"])
        .output().expect("add");
    ...
    let out = Command::new(env!("CARGO_BIN_EXE_req"))
        ...
        .output().expect("list");
    assert!(!out.status.success(), "list should refuse after per-file tamper");
...
fn req_0076_near_clone_triggers_dup_intent_warning() {
    ...
    let _ = s.run(&[
        "add", "--title", "Persist user sessions across restarts forever",
        "--statement", "The system shall persist user sessions across process restarts.",
        "--rationale", "Users lose work today.",
        "--kind", "functional", "--priority", "should",
        "--accept", "Session survives restart in fixture",
    ]);
    // Near-clone: same intent, very similar wording
    let _ = s.run(&[
        "add", "--title", "Persist user sessions across process restarts",
        "--statement", "The system shall persist user sessions across process restarts always.",
        "--rationale", "Same intent, different words.",
        "--kind", "functional", "--priority", "should",
        "--accept", "Session survives restart in fixture as well",
    ]);
    let out = s.run(&["validate"]);
    let text = String::from_utf8_lossy(&out.stdout);
    assert!(text.contains("REQ-V-0020"), "expected duplicate-intent warning, got:\n{}", text);
}
...
fn req_0077_verifies_link_without_test_record_warns() {
    ...
    for i in 1..=2 {
        s.run(&[
            "add", "--title", &format!("Subject of the verification {}", i),
            "--statement", "The system shall have this perfectly fine baseline behaviour.",
    ...
    let out = s.run(&["validate"]);
    let text = String::from_utf8_lossy(&out.stdout);
    assert!(text.contains("REQ-V-0019"), "expected verifies-without-evidence warning, got:\n{}", text);
}
...
fn req_0078_schema_batch_describes_oneof_mutations() {
    ...
    assert!(mutations.is_array(), "batch schema should describe mutation alternatives");
...
fn req_0079_audit_gate_exits_nonzero_without_signing() {
    ...
    let s = Sandbox::new(); s.init("p");
    let dir = s.dir.path();
    let _ = std::process::Command::new("git").current_dir(dir).args(["init", "-q", "-b", "main"]).output();
    let _ = std::process::Command::new("git").current_dir(dir).args(["config", "user.email", "t@example.com"]).output();
    let _ = std::process::Command::new("git").current_dir(dir).args(["config", "user.name", "T"]).output();
    let _ = std::process::Command::new("git").current_dir(dir).args(["add", "project.req"]).output();
    let _ = std::process::Command::new("git").current_dir(dir).args(["commit", "-q", "-m", "init"]).output();
    let out = std::process::Command::new(env!("CARGO_BIN_EXE_req"))
        .current_dir(dir)
        .args(["--file", s.path().to_str().unwrap(), "audit", "--gate", "--require-good-signature"])
        .output().expect("audit gate");
