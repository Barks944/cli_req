...
fn add_req(s: &Sandbox, title: &str, statement: &str, kind: &str, accepts: &[&str]) -> std::process::Output {
    let mut args = vec![
        "add",
        "--title", title,
        "--statement", statement,
        "--rationale", "Set up for slice A tests.",
        "--kind", kind,
        "--priority", "could",
    ];
    for a in accepts { args.push("--accept"); args.push(*a); }
    s.run(&args)
}
...
fn req_0061_license_file_is_mit() {
    ...
    assert!(text.contains("MIT License"), "LICENSE should say MIT License");
    assert!(text.contains("Permission is hereby granted"), "MIT body missing");
}

#[test]
fn req_0061_cargo_declares_license() {
    let text = fs::read_to_string("Cargo.toml").expect("Cargo.toml present");
    assert!(text.contains("license = \"MIT\""), "Cargo.toml should declare license = \"MIT\"");
}
...
fn req_0074_help_has_format_policy_section() {
    ...
    assert!(out.status.success(), "req help format-policy should succeed");
    let body = stdout(&out);
    assert!(body.contains("req migrate"), "policy should mention req migrate");
    assert!(body.contains("_format"), "policy should describe _format tag");
}
...
fn req_0074_help_index_lists_format_policy() {
    ...
    let body = stdout(&out);
    assert!(body.contains("format-policy"), "help index should list format-policy section");
}
...
fn req_0073_obsolete_hidden_from_default_list() {
    ...
    add_req(&s, "Live and well thing", "The system shall keep this active forever.", "constraint", &[]);
    add_req(&s, "Retire this old one", "The system shall keep this one for a while.", "constraint", &[]);
    ...
    assert!(!out.contains("REQ-0002"), "obsolete req should be hidden by default, got: {}", out);
}
...
fn req_0073_include_obsolete_flag_brings_them_back() {
    ...
    assert!(out.contains("REQ-0002"), "--include-obsolete should bring obsolete back: {}", out);
}
...
fn req_0073_status_obsolete_filter_still_works() {
    ...
    assert!(out.contains("REQ-0002"), "explicit --status obsolete should show: {}", out);
    assert!(!out.contains("REQ-0001"), "explicit --status obsolete should NOT show live reqs: {}", out);
}
...
fn req_0065_coverage_strict_exits_nonzero_with_orphans() {
    let s = Sandbox::new(); s.init("p");
    add_req(&s, "Orphan candidate one", "The system shall have one untested requirement.", "constraint", &[]);
    let out = s.run(&["coverage", "--path", s.dir.path().to_str().unwrap(), "--strict"]);
    assert!(!out.status.success(), "orphan should make strict mode fail; stderr={}", stderr(&out));
}
...
fn req_0065_coverage_default_remains_informational() {
    ...
    assert!(out.status.success(), "default mode should be zero-exit even with orphans");
}
...
fn req_0065_coverage_strict_passes_when_clean() {
    let s = Sandbox::new(); s.init("p");
    let out = s.run(&["coverage", "--path", s.dir.path().to_str().unwrap(), "--strict"]);
    assert!(out.status.success(), "empty project has no orphans, strict should pass");
}
...
fn req_0072_add_from_json_validator_still_rejects() {
    ...
    assert!(!out.status.success(), "validator should still reject bad input from JSON");
    assert!(stderr(&out).contains("title is too short") || stderr(&out).contains("modal verb")
...
fn req_0071_hooks_install_writes_gitattributes_pin() {
    ...
    let out = std::process::Command::new(env!("CARGO_BIN_EXE_req"))
        .args(["hooks", "install", "--repo", s.dir.path().to_str().unwrap(), "--force"])
        .output().expect("invoke req");
    assert!(out.status.success(), "stderr={}", String::from_utf8_lossy(&out.stderr));
    let attrs = fs::read_to_string(s.dir.path().join(".gitattributes")).expect(".gitattributes written");
    assert!(attrs.contains("project.req -text eol=lf"), "should pin project.req: {}", attrs);
    assert!(attrs.contains("*.req merge=req-merge"), "should keep merge driver: {}", attrs);
}
...
fn req_0071_hooks_install_is_idempotent() {
    ...
    assert_eq!(pin_count, 1, "pin should appear exactly once, got {}: {}", pin_count, attrs);
}
