...
fn add_req(file: &Path, title: &str, statement: &str, accept: &str) {
    let out = req(&[
        ...
        statement,
        "-r",
        "Rationale that is long enough to satisfy the linter.",
        "-a",
        accept,
    ]);
    assert!(
...
fn setup() -> Trio {
    ...
    add_req(
        ...
        "The system shall do the base thing.",
        "base thing happens",
    );
...
fn req_0207_clean_merge_keeps_both_additions() {
    ...
    assert!(
        listing.contains("Shared base requirement"),
        "base lost:\n{listing}"
    );
    assert!(
        listing.contains("Ours adds login"),
        "ours addition lost:\n{listing}"
    );
    assert!(
        listing.contains("Theirs adds logout"),
        "theirs addition lost:\n{listing}"
    );
    // The merged file must load with a valid integrity hash (a re-listing here
    // would fail if it didn't).
    assert!(
        req(&["--file", p(&t.merged), "list"]).status.success(),
        "merged file failed to load"
    );
}
...
fn req_0207_field_level_merge_of_same_artifact() {
    ...
    assert!(
        out.status.success(),
        "distinct-field edits should merge cleanly: {}",
        String::from_utf8_lossy(&out.stderr)
    );

    let show = String::from_utf8_lossy(&req(&["--file", p(&t.merged), "show", "REQ-0001"]).stdout)
        .into_owned();
    assert!(
        show.contains("Ours edited the title"),
        "ours title edit lost:\n{show}"
    );
    assert!(
        show.to_lowercase().contains("must"),
        "theirs priority edit lost:\n{show}"
    );
}
...
fn sr_0010_same_field_conflict_exits_nonzero_and_preserves_both() {
    ...
    assert!(
        !out.status.success(),
        "a same-field divergence must fail loud, not silently merge"
    );

    let merged = std::fs::read_to_string(&t.merged).unwrap();
    assert!(
        merged.contains("OURS distinct title"),
        "our side dropped:\n{merged}"
    );
    assert!(
        merged.contains("THEIRS distinct title"),
        "their side dropped:\n{merged}"
    );
    assert!(
    ...
    assert!(
        ...
        "conflict not reported on stderr: {stderr}"
    );
}
...
fn sr_0010_edit_vs_delete_conflicts() {
    ...
    let merged = std::fs::read_to_string(&t.merged).unwrap();
    assert!(
        merged.contains("THEIRS kept and edited"),
        "edited side dropped on edit-vs-delete:\n{merged}"
    );
}
...
fn sr_0010_hooks_registers_safe_driver() {
    ...
    let gi = std::process::Command::new("git")
        .arg("init")
        .arg(dir.path())
        .output()
        .expect("git init");
    ...
    assert!(
        combined.contains("req merge --base"),
        "safe driver not registered:\n{combined}"
    );
    assert!(
        !combined.contains("|| true"),
        "driver still masks its exit status:\n{combined}"
    );
    assert!(
        !combined.contains("renumber --base %O"),
        "still registers the misconfigured renumber driver:\n{combined}"
    );
}
